...
export default function App() {
  ...
  const heroYRaw = useTransform(scrollY, [0, 800], [0, 240]);
  const heroScaleRaw = useTransform(scrollY, [0, 800], [1, 1.08]);
  const heroOpacityRaw = useTransform(scrollY, [0, 600], [1, 0.8]);
    ...
    <div className={`font-sans scroll-smooth selection:bg-black selection:text-white ${dark ? 'bg-neutral-950 text-neutral-100' : 'bg-white text-neutral-900'}`}>
      ...
      <section id="home" className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-4 py-28 md:py-40">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0, transition: { duration: 0.6, ease: "easeOut" } }} viewport={{ once: true, margin: "-80px" }} className="max-w-2xl">
      ...
      <section className={`border-y ${dark ? 'border-white/10 bg-neutral-900/30' : 'border-neutral-200/70 bg-neutral-50'}`}>
        <div className="mx-auto max-w-7xl px-4 py-6 grid grid-cols-2 md:grid-cols-4 gap-6 text-xs text-neutral-600">
          {[
            ...
            <motion.div key={t} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0, transition: { duration: 0.6, ease: "easeOut" } }} viewport={{ once: true }} className="flex items-center gap-3">
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((s, i) => (
            <motion.div
              key={s.title}
              initial={{ opacity: 0, y: 24 }}
              whileInView={{ opacity: 1, y: 0, transition: { duration: 0.6, ease: "easeOut", delay: i * 0.05 } }}
              viewport={{ once: true }}
      ...
      <section id="process" className="mx-auto max-w-7xl px-4 py-20">
        ...
        <ol className="mt-10 grid grid-cols-1 md:grid-cols-4 gap-6 text-sm">
          {[
            ...
            <motion.li key={step.n} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0, transition: { duration: 0.6, ease: "easeOut", delay: i * 0.05 } }} viewport={{ once: true }} className="rounded-2xl border border-neutral-200 p-5">
      ...
      <section id="contact" className="relative mx-auto max-w-7xl px-4 py-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0, transition: { duration: 0.6, ease: "easeOut" } }} viewport={{ once: true }}>
          ...
          <motion.form initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0, transition: { duration: 0.6, ease: "easeOut" } }} viewport={{ once: true }} className={`rounded-2xl border p-6 shadow-sm ${dark ? 'border-white/10 bg_white/5' : 'border-neutral-200 bg-white'}`} onSubmit={(e)=>e.preventDefault()}>
            <div className="grid grid-cols-1 gap-4">
              <label className="text-sm">Name<input required className={`mt-1 w-full rounded-lg border px-3 py-2 outline-none focus:ring-2 ${dark ? 'border-neutral-700 focus:ring-white/30 bg-neutral-900 text-white' : 'border-neutral-300 focus:ring-neutral-900'}`} /></label>
              <label className="text-sm">Email<input type="email" required className={`mt-1 w-full rounded-lg border px-3 py-2 outline-none focus:ring-2 ${dark ? 'border-neutral-700 focus:ring-white/30 bg-neutral-900 text-white' : 'border-neutral-300 focus:ring-neutral-900'}`} /></label>
              <label className="text-sm">Phone<input className={`mt-1 w-full rounded-lg border px-3 py-2 outline-none focus:ring-2 ${dark ? 'border-neutral-700 focus:ring-white/30 bg-neutral-900 text-white' : 'border-neutral-300 focus:ring-neutral-900'}`} /></label>
              <label className="text-sm">Message<textarea rows={4} className={`mt-1 w-full rounded-lg border px-3 py-2 outline-none focus:ring-2 ${dark ? 'border-neutral-700 focus:ring-white/30 bg-neutral-900 text-white' : 'border-neutral-300 focus:ring-neutral-900'}`} placeholder="Tell us about your project..." /></label>
      ...
      <a href="#contact" className="hidden md:inline-flex fixed bottom-6 right-6 rounded-full bg-neutral-900 text-white px-5 py-3 shadow-lg">Book a Consultation</a>
