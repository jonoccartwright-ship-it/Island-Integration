...
export default function IslandIntegrationSite() {
  ...
  // White theme only
  const logoSrc = `${import.meta.env.BASE_URL}logo-light.png`;
  ...
  return (
    <div className="font-sans scroll-smooth selection:bg-black selection:text-white bg-white text-neutral-900">
      {/* Header */}
      <header className="sticky top-0 z-50 backdrop-blur border-b bg-white/70 border-neutral-200/60">
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between">
          <a href="#home" className="flex items-center gap-3">
            <span className="inline-flex items-center justify-center rounded-xl p-1.5 md:p-2 bg-black/5 ring-1 ring-black/5">
              <img src={logoSrc} alt="Island Integration logo" className="h-10 md:h-12 w-auto drop-shadow-sm" />
            </span>
        ...
        {open && (
          <div className="md:hidden fixed inset-0 z-50">
            ...
            <nav className="absolute right-0 top-0 h-full w-72 shadow-xl p-6 flex flex-col gap-4 bg-white">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className="inline-flex items-center justify-center rounded-lg p-1.5 bg-black/5 ring-1 ring-black/5">
                    <img src={logoSrc} alt="Island Integration logo" className="h-8 w-auto drop-shadow-sm" />
                  </span>
        ...
        <div className="mx-auto max-w-7xl px-4 py-28 md:py-40">
          <motion.div variants={fadeUp} initial="initial" whileInView="animate" viewport={{ once: false, margin: "-80px" }} className="max-w-2xl">
            <div className="mb-6 flex items-center gap-3">
              <span className="inline-flex items-center justify-center rounded-2xl p-2 md:p-3 bg-black/5 ring-1 ring-black/5 backdrop-blur-sm">
                <img src={logoSrc} alt="Island Integration logo" className="h-16 md:h-20 w-auto drop-shadow-sm" />
              </span>
              <span className="sr-only">Island Integration</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-semibold leading-tight">
              <span className="bg-gradient-to-r bg-clip-text text-transparent from-neutral-900 to-neutral-600">Smart Homes & Luxury Integration</span>
              <span className="block text-neutral-600 text-xl md:text-2xl mt-4">Bahamas-based Control4, networking, AV, and security specialists.</span>
            </h1>
      ...
      <section id="services" className="mx-auto max-w-7xl px-4 py-20">
        <div className="flex items-end justify-between gap-6">
          <div>
            ...
            <div className="h-1 w-24 bg-gradient-to-r rounded-full mt-3 from-neutral-900 to-neutral-500" />
          </div>
          <a href="#contact" className="text-sm underline underline-offset-4">Request a site visit</a>
        </div>
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((s, i) => (
            <motion.div key={s.title} variants={fadeUp} initial="initial" whileInView="animate" viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="group rounded-2xl border backdrop-blur p-6 shadow-sm hover:shadow-lg transition relative overflow-hidden border-neutral-200/70 bg-white/70">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <motion.div variants={fadeUp} initial="initial" whileInView="animate" viewport={{ once: true }}>
            ...
            <div className="mt-8 flex items-center gap-6">
              <div className="h-28 w-28 rounded flex items-center justify-center text-[10px] bg-neutral-200 text-neutral-500">
                QR Code
              </div>
          ...
          <motion.form variants={fadeUp} initial="initial" whileInView="animate" viewport={{ once: true }} className="rounded-2xl border p-6 shadow-sm border-neutral-200 bg-white" onSubmit={(e) => e.preventDefault()}>
            <div className="grid grid-cols-1 gap-4">
              <label className="text-sm">Name
                <input required className="mt-1 w-full rounded-lg border px-3 py-2 outline-none focus:ring-2 border-neutral-300 focus:ring-neutral-900" />
              </label>
              <label className="text-sm">Email
                <input type="email" required className="mt-1 w-full rounded-lg border px-3 py-2 outline-none focus:ring-2 border-neutral-300 focus:ring-neutral-900" />
              </label>
              <label className="text-sm">Phone
                <input className="mt-1 w-full rounded-lg border px-3 py-2 outline-none focus:ring-2 border-neutral-300 focus:ring-neutral-900" />
              </label>
              <label className="text-sm">Message
                <textarea rows={4} className="mt-1 w-full rounded-lg border px-3 py-2 outline-none focus:ring-2 border-neutral-300 focus:ring-neutral-900" placeholder="Tell us about your project..." />
              </label>
