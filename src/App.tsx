...
export default function IslandIntegrationSite() {
  ...
  useEffect(()=>{const m=matchMedia("(prefers-reduced-motion: reduce)");setReduceMotion(m.matches);const f=()=>setReduceMotion(m.matches);m.addEventListener?.("change",f);return()=>m.removeEventListener?.("change",f)},[]);
  ...
  useEffect(()=>{const exp=["Lighting Control","Home Cinema","Automated Shading","Design","Climate Control","Multi-Room TV & Audio","Smart Home Security","WiFi & Networking","System Support"];console.assert(services.length===9);exp.forEach((t,i)=>console.assert(services[i]?.title===t));console.assert(/^mailto:/i.test(mailtoHref));},[]);
  const fadeUp={initial:{opacity:0,y:24},animate:{opacity:1,y:0,transition:{duration:0.6,ease:"easeOut"}}} as const;
  return (<div className={`font-sans scroll-smooth selection:bg-black selection:text-white ${dark?'bg-neutral-950 text-neutral-100':'bg-white text-neutral-900'}`}>
    <header className={`sticky top-0 z-50 backdrop-blur border-b ${dark?'bg-neutral-950/70 border-white/10':'bg-white/70 border-neutral-200/60'}`}>
      <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between">
        <a href="#home" className="flex items-center gap-3">
          <img src={logoSrc} alt="Island Integration logo" className="h-8 w-auto"/><span className="font-semibold tracking-wide">Island Integration</span>
        </a>
        <nav className="hidden md:flex items-center gap-8 text-sm"><a href="#services" className="hover:opacity-60">Services</a><a href="#process" className="hover:opacity-60">Process</a><a href="#contact" className="hover:opacity-60">Contact</a></nav>
        <button aria-label="Toggle theme" onClick={()=>setDark(d=>!d)} className={`mr-2 hidden sm:inline-flex items-center gap-2 rounded-full border px-3 py-2 text-xs ${dark?'border-white/15 hover:bg-white/5':'border-neutral-300/70 hover:bg-neutral-100'}`}>{dark?'🌙 Dark':'☀️ Light'}</button>
        <button aria-label="Open menu" className="md:hidden rounded-lg border border-neutral-300 px-3 py-2" onClick={()=>setOpen(true)}><div className="h-0.5 w-5 bg-current mb-1"/><div className="h-0.5 w-5 bg-current"/></button>
      </div>
      {open&&(<div className="md:hidden fixed inset-0 z-50"><div className="absolute inset-0 bg-black/40" onClick={()=>setOpen(false)}/>
        <nav className={`absolute right-0 top-0 h-full w-72 shadow-xl p-6 flex flex-col gap-4 ${dark?'bg-neutral-900 text-white':'bg-white'}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3"><img src={logoSrc} alt="Island Integration logo" className="h-6 w-auto"/><span className="font-semibold">Island Integration</span></div>
    ...
    <section id="home" ref={heroRef} className="relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-4 py-28 md:py-40">
        <motion.div variants={fadeUp} initial="initial" whileInView="animate" viewport={{once:false, margin:"-80px"}} className="max-w-2xl">
          <div className="mb-6 flex items-center gap-3"><img src={logoSrc} alt="Island Integration logo" className="h-10 w-auto"/><span className="sr-only">Island Integration</span></div>
          <h1 className="text-4xl md:text-6xl font-semibold leading-tight"><span className={`bg-gradient-to-r bg-clip-text text-transparent ${dark?'from-white to-neutral-400':'from-neutral-900 to-neutral-600'}`}>Smart Homes & Luxury Integration</span><span className="block text-neutral-500 text-xl md:text-2xl mt-4">Bahamas‑based Control4, networking, AV, and security specialists.</span></h1>
          <div className="mt-8 flex flex-col sm:flex-row items-start gap-4"><a href="#services" className="rounded-full border border-neutral-900 px-6 py-3">Our Services</a></div>
          <p className="mt-8 text-sm text-neutral-600 max-w-lg">12+ years delivering seamless control, resilient networks, and beautiful audio/visual experiences across Nassau, New Providence, and the outer islands.</p>
        </motion.div>
      </div>
    </section>
    ...
    <section className={`border-y ${dark?'border-white/10 bg-neutral-900/30':'border-neutral-200/70 bg-neutral-50'}`}>
      <div className="mx-auto max-w-7xl px-4 py-6 grid grid-cols-2 md:grid-cols-4 gap-6 text-xs text-neutral-600">
        {["Control4 Partner","Snap One / WattBox","Luma Cameras","Ubiquiti Networks"].map(t=>(
          <motion.div key={t} variants={fadeUp} initial="initial" whileInView="animate" viewport={{once:true}} className="flex items-center gap-3"><span className="h-2 w-2 rounded-full bg-neutral-900"/>{t}</motion.div>
        ))}
      </div>
    </section>
    ...
    <section id="services" className="mx-auto max-w-7xl px-4 py-20">
      <div className="flex items-end justify-between gap-6">
        <div><h2 className="text-2xl md:text-4xl font-semibold">Services</h2><div className={`h-1 w-24 bg-gradient-to-r rounded-full mt-3 ${dark?'from-white to-neutral-400':'from-neutral-900 to-neutral-500'}`}/></div>
        <a href="#contact" className="text-sm underline underline-offset-4">Request a site visit</a>
      </div>
      <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.map((s,i)=>(
          <motion.div key={s.title} variants={fadeUp} initial="initial" whileInView="animate" viewport={{once:true}} transition={{delay:i*0.05}}
            ...
            <div className="flex items-center justify-between">
              <div className="p-2 rounded-xl bg-neutral-100 text-neutral-900"><svg viewBox="0 0 24 24" fill="none" className="h-6 w-6"><path d="M3 7a9 9 0 0 1 18 0M6 10a6 6 0 1 1 12 0M9 13a3 3 0 1 1 6 0" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg></div>
    ...
    <section id="process" className="mx-auto max-w-7xl px-4 py-20">
      <div><h2 className="text-2xl md:text-4xl font-semibold">Our Process</h2><div className={`h-1 w-24 bg-gradient-to-r rounded-full mt-3 ${dark?'from-white to-neutral-400':'from-neutral-900 to-neutral-500'}`}/></div>
    ...
    <section id="contact" className="relative mx-auto max-w-7xl px-4 py-20">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <motion.div variants={fadeUp} initial="initial" whileInView="animate" viewport={{once:true}}>
          ...
          <p className="mt-4 text-sm text-neutral-600 max-w-md">Call <a href="tel:+12428209013" className="underline">+1 (242) 820‑9013</a> or message us. We typically reply same day.</p>
          <div className="mt-8 flex items-center gap-6">
            <div className={`h-28 w-28 rounded flex items-center justify-center text-[10px] ${dark?'bg-neutral-800 text-neutral-400':'bg-neutral-200 text-neutral-500'}`}>QR Code</div>
            <div className="text-sm text-neutral-600">Scan to visit <span className="font-medium">island‑integration.com</span><br/>or email <a href="mailto:info@island-integration.com" className="underline">info@island‑integration.com</a></div>
          </div>
        </motion.div>
    ...
    <footer className={`border-t ${dark?'border-white/10':'border-neutral-200/70'}`}>
      <div className="mx-auto max-w-7xl px-4 py-10 text-sm text-neutral-600 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-6"><a href="#services" className="hover:underline">Services</a><a href="#process" className="hover:underline">Process</a><a href="#contact" className="hover:underline">Contact</a></div>
      </div>
    </footer>

    <a href={mailtoHref} target="_top" rel="noopener noreferrer" className="hidden md:inline-flex fixed bottom-6 right-6 rounded-full bg-neutral-900 text-white px-5 py-3 shadow-lg" title="Email Island Integration">Book a Consultation</a>
